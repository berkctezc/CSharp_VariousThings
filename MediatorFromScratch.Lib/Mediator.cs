...
public class Mediator(Func<Type, object> serviceResolver, IDictionary<Type, Type> handlerDetails)
    : IMediator
{
    public async Task<TResponse> SendAsync<TResponse>(IRequest<TResponse> request)
    {
        var requestType = request.GetType();
        if (!handlerDetails.ContainsKey(requestType))
        ...
        return await (
            (Task<TResponse>)
                handler.GetType().GetMethod("HandleAsync")!.Invoke(handler, new[] { request })!
        )!;
    }
}
